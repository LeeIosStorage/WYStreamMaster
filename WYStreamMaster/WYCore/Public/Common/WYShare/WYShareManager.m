...
- (instancetype)init{
    self = [super init];
    if (self) {
        [WXApi registerApp:WX_ID withDescription:@"YR"];
...
- (BOOL)shareToWXWithImage:(UIImage *)wxImage scene:(int)scene{
    if (!([WXApi isWXAppInstalled])) {
        ...
        [WYCommonUtils showAlertWithMsg:@"微信分享失败！"];
        return NO;
    }
...
//        msg.description = msg.title;
    ...
    if (!wxImage) {
        wxImage = [UIImage imageNamed:@"login_logo_icon"];
    }
...
- (void)shareToQQWithImage:(UIImage *)qqImage{
    if (!([QQApiInterface isQQInstalled])) {
        [WYCommonUtils showAlertWithMsg:@"QQ分享失败！"];
        return;
    }
    NSData *data;
...
//        }
    ...
    if (!image) {
        image = [UIImage imageNamed:@"login_logo_icon"];
    }
    if (image) {
...
- (void)shareToWb:(WYWeiboShareResultBlock)result title:(NSString *)title description:(NSString *)description webpageUrl:(NSString *)webpageUrl image:(UIImage*)image VC:(id)VC isVideo:(BOOL)isVideo{
    if (!([WeiboSDK isWeiboAppInstalled])) {
        [WYCommonUtils showAlertWithMsg:@"微博分享失败！"];
        return;
    }
...
//    }
    ...
    NSString* shareTitle = [NSString stringWithFormat:@"%@  (分享自@%@) %@",title,appBundleName,webpageUrl];
...
- (void)didReceiveWeiboResponse:(WBBaseResponse *)response {
    ...
    if ([response isKindOfClass:WBAuthorizeResponse.class])
    {
        ...
        if (response.statusCode == WeiboSDKResponseStatusCodeSuccess) {
            ...
            [self shareSuccessWithTitle:@"分享微博成功"];
        }else{
            [self shareErrorWithTitle:@"分享微博失败"];
        }
    }
}
    ...
    if ([resp isKindOfClass:[QQBaseResp class]]) {
        ...
        switch (resp1.type)
        {
            case ESENDMESSAGETOQQRESPTYPE:
            {
                ...
                if ([sendResp.result intValue] == EQQAPISENDSUCESS) {
                    [self shareSuccessWithTitle:@"分享QQ成功"];
                }else{
                    [self shareErrorWithTitle:@"分享QQ失败"];
                }
                break;
            }
            default:
        ...
        if([resp1 isKindOfClass:[SendMessageToWXResp class]]){
            ...
            switch (resp1.errCode) {
                case WXSuccess:{
                    [self shareSuccessWithTitle:@"分享微信成功"];
                }
                    break;
                    
                default:
                    [self shareErrorWithTitle:@"分享微信失败"];
                    break;
            }
...
#pragma mark - custom
-(void)shareErrorWithTitle:(NSString *)title{
    [MBProgressHUD showError:title];
}

- (void)shareSuccessWithTitle:(NSString *)title{
