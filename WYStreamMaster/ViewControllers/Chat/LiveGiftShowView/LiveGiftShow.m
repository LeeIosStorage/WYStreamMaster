...
/**
 ...
 @param model 传入模型数据 从传入的开始计算数字计算
 */
- (void)addGiftListModel:(LiveGiftShowModel *)model{
    ...
    if (!showView && ![showView isKindOfClass:[LiveGiftShowView class]]) {//如果不存在该用户 新建
        ...
        view.model = model;
        [view addGiftNumberFrom:[model.giftModel.goldCount integerValue]];
        ...
        [showView addGiftNumberFrom:[model.giftModel.goldCount integerValue]];
        if ([self.giftViewArr containsObject:kGiftViewRemoved]) {
