...
//    if (model.is_read) {
//        self.isReadMessageImage.hidden = YES;
//    } else {
//        self.isReadMessageImage.hidden = NO;
//    }
    self.isReadMessageImage.hidden = YES;

    if ([model.sex isEqualToString:@"0"]) {
        // 男
        [self.sexImageView setImage:[UIImage imageNamed:@"home_male"]];
    } else {
        // 女
        [self.sexImageView setImage:[UIImage imageNamed:@"home_female"]];
    }
}
