...
- (void)notificationSocketConnect:(NSNotification *)notification
{
    ...
    if ([socketCmmid isEqualToString:@"100001"]) {
        ...
        NSString *content = @"进入了直播间";
        NSString *extraString = notiDict[@"data"][@"extInfo"][@"nickname"];        [self.roomView.chatroomControl sendLocalMessageWithExtraString:extraString content:[NSString stringWithFormat:@"%@ %@", extraString, content] extraColor:[UIColor whiteColor]];
//        NSString *extraString = notiDict[@"data"][@"extInfo"][@"nickname"];        [self.roomView.chatroomControl sendMessageWithNotificationText:[NSString stringWithFormat:@"%@进入了放间", extraString]];
